...
import { Loader2, Upload, X, Camera, Images, ImagePlus, Phone, CheckCircle, Shield, Crown, Users, Share2 } from "lucide-react";
...
const ProfileTab = () => {
  ...
  const handleOpenToInvitationsChange = async (checked: boolean) => {
    ...
    try {
      ...
      toast({
        ...
        description: checked ? "Brands can now see you're open to free collaborations" : "Your profile no longer shows the open to invitations badge",
      });
    } catch (error) {
      setProfile({ ...profile, open_to_invitations: !checked });
  ...
  const removeCoverImage = async (index: number) => {
    if (index === 0) { toast({ title: "Cannot remove", description: "Cover image 1 is required", variant: "destructive" }); return; }
    const coverKey = index === 1 ? 'cover_image_url_2' : 'cover_image_url_3';
    if (!profile[coverKey]) return;
    setProfile({ ...profile, [coverKey]: "" });
    toast({ title: "Removed", description: `Cover image ${index + 1} removed` });
  };

  const toggleCategory = (category: string) => {
    setProfile({
      ...profile,
      categories: profile.categories.includes(category) ? profile.categories.filter((c) => c !== category) : [...profile.categories, category],
    });
  };

  const handleSendPhoneOtp = async () => {
    if (newPhoneNumber.length < 10) { toast({ title: "Invalid Phone", description: "Please enter a valid phone number with country code", variant: "destructive" }); return; }
    setSendingOtp(true);
    try {
      const { error } = await supabase.auth.signInWithOtp({ phone: newPhoneNumber });
      if (error) throw error;
  ...
  const handleVerifyPhoneOtp = async () => {
    if (phoneOtp.length !== 6) { toast({ title: "Invalid Code", description: "Please enter the 6-digit verification code", variant: "destructive" }); return; }
    setVerifyingOtp(true);
    try {
      const { error } = await supabase.auth.verifyOtp({ phone: newPhoneNumber, token: phoneOtp, type: 'sms' });
      if (error) throw error;
      const { error: updateError } = await supabase.from("creator_profiles").update({ phone_number: newPhoneNumber, phone_verified: true }).eq("id", profile.id);
      if (updateError) throw updateError;
      setProfile({ ...profile, phone_number: newPhoneNumber, phone_verified: true });
      setIsEditingPhone(false); setNewPhoneNumber(""); setPhoneOtp("");
      toast({ title: "Phone Updated", description: "Your phone number has been verified and updated" });
    } catch (error: any) {
  ...
  const handleSave = async () => {
    if (!profile.display_name.trim()) { toast({ title: "Validation Error", description: "Display name is required", variant: "destructive" }); return; }
    setSaving(true);
    try {
      const { error } = await supabase
        .from("creator_profiles")
      ...
      <Sheet open={isEditDrawerOpen} onOpenChange={setIsEditDrawerOpen}>
        <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto p-0">
          ...
          <div className="px-6 py-4 pb-24">
            <Accordion type="multiple" defaultValue={["media"]} className="rounded-lg border bg-card text-card-foreground shadow-sm divide-y">
              {/* ── Your Media ── */}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        ...
                        <Label htmlFor="profile-image-upload" className="absolute -bottom-1 -right-1 p-1.5 bg-primary text-primary-foreground rounded-full cursor-pointer hover:bg-primary/90 transition-colors shadow-md">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[0, 1, 2].map((index) => {
                          const coverUrl = index === 0 ? profile.cover_image_url : index === 1 ? profile.cover_image_url_2 : profile.cover_image_url_3;
                              ...
                              {coverUrl ? (
                                <div className="relative aspect-[3/2] rounded-lg overflow-hidden border group">
                                  ...
                                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                                    ...
                                    {index > 0 && (
                                      <button onClick={() => removeCoverImage(index)} className="px-2 py-1 bg-destructive text-destructive-foreground rounded hover:bg-destructive/90 text-xs font-medium">Remove</button>
                                    )}
                                  </div>
                                </div>
                              ) : (
                                <Label htmlFor={inputId} className="flex flex-col items-center justify-center aspect-[3/2] rounded-lg border-2 border-dashed border-muted-foreground/25 cursor-pointer hover:border-primary/50 hover:bg-muted/30 transition-colors">
                                  {isUploading ? <Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /> : (
                                    <>
                                      ...
                                      <span className="text-[10px] text-muted-foreground text-center px-1">{index === 0 ? "Cover" : `Photo ${index + 1}`}</span>
                                    </>
                                  )}
                                </Label>
                              )}
                  ...
                  <div className="space-y-3">
                    <div className="space-y-3">
                      <div className="space-y-1.5">
                        ...
                        <Input id="display_name" value={profile.display_name} onChange={(e) => setProfile({ ...profile, display_name: e.target.value })} placeholder="Your display name" className="h-9" />
                      </div>
                      <div className="space-y-1.5">
                        <Label htmlFor="bio" className="text-xs">Bio</Label>
                        <Textarea id="bio" value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} placeholder="Tell us about yourself..." rows={3} className="min-h-[70px]" />
                    ...
                    <div className="space-y-2">
                      ...
                      {profile.phone_number && profile.phone_verified && !isEditingPhone ? (
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="ghost" size="sm" className="ml-auto h-7 text-xs" onClick={() => { setIsEditingPhone(true); setNewPhoneNumber(""); setPhoneOtp(""); }}>Change</Button>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          {!profile.phone_number && !isEditingPhone && <p className="text-xs text-amber-600 dark:text-amber-400">No phone number on file.</p>}
                          ...
                          {isEditingPhone && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => { setIsEditingPhone(false); setNewPhoneNumber(""); setPhoneOtp(""); }}>Cancel</Button>}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {AVAILABLE_CATEGORIES.map((category) => (
                          <Badge key={category} variant={profile.categories.includes(category) ? "default" : "outline"} className="cursor-pointer text-xs h-6" onClick={() => toggleCategory(category)}>
                      ...
                      <div className="space-y-1 pt-1">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {LANGUAGES.filter(l => l !== profile.primary_language).map((lang) => (
                            <Badge key={lang} variant={profile.secondary_languages.includes(lang) ? "default" : "outline"} className="cursor-pointer text-xs h-6"
                              onClick={() => {
              ...
              {/* ── VIP Badge ── */}
              <AccordionItem value="vip" className="border-b-0">
                <AccordionTrigger className="px-4 py-3 hover:no-underline">
                  <span className="flex items-center gap-2 text-sm font-medium">
          ...
          {/* Sticky Save inside Sheet */}
          <div className="sticky bottom-0 bg-background/95 backdrop-blur-sm border-t p-4 flex justify-end">
