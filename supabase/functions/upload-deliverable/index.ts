...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!file || !bookingId) {
      return new Response(JSON.stringify({ error: 'File and booking_id are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (booking.status !== 'accepted' && booking.status !== 'completed') {
      return new Response(JSON.stringify({ error: 'Can only upload deliverables for accepted bookings' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const r2Key = `deliverables/${bookingId}/${version}/${timestamp}-${sanitizedFileName}`;
    ...
    // Insert record into booking_deliverables
    const { data: deliverableRecord, error: insertError } = await supabase
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to save deliverable record' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(`Deliverable uploaded: ${r2Key}, booking: ${bookingId}, version: ${version}`);
